...
require_once 'WGM/Form/Renderer.php';

require_once(realpath('themes/engines/smarty/SmartyTemplate.php'));

class WGM_Form extends HTML_QuickForm_Page {
   ...
   */
  static $CB_PREFIX     = 'mark_x_';
  static $CB_PREFIX_LEN = 7;
  static $CB_PREFIY     = 'mark_y_';
  static $CB_PREFIY_LEN = 7;
  static $CB_PREFIZ     = 'mark_z_';
  static $CB_PREFIZ_LEN = 7;
  ...
  function registerRules( ) {
    $this->registerRule( 'name'       , 'callback', 'name'       , 'WGM_Validate' );
    $this->registerRule( 'variable'   , 'callback', 'variable'   , 'WGM_Validate' );
    $this->registerRule( 'phoneNumber', 'callback', 'phoneNumber', 'WGM_Validate' );
    $this->registerRule( 'queryString', 'callback', 'queryString', 'WGM_Validate' );
    $this->registerRule( 'url'        , 'callback', 'url'        , 'WGM_Validate' );
  }
  ...
  function add($type, $name, $label,
    ...
    if (HTML_QuickForm::isError($element)) {
      WGM_Error::abort(HTML_QuickForm::errorMessage($element));
    }
   ...
   * here and expect all WGM forms to build their form in the function
   * buildQuickForm.
   *
   */
  function buildForm() {
  ...
  function isOneState( ) {
    return $this->_state->getType( ) & ( WGM_State::SFINAL | WGM_State::INITIAL );
  }
  ...
  function getRenderer() {
    if (isset($this->renderer)) {
      ...
      $this->renderer = new WGM_Form_Renderer($template);
      return $this->renderer;
    }
  }
