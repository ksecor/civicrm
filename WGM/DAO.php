...
class WGM_DAO extends DB_DataObject {
  ...
  function setFactory(&$factory) {
    global $WGM_DAO_Factory;

    if ( !isset( $WGM_DAO_Factory ) ) {
      $WGM_DAO_Factory =& $factory;
    }
  }
  ...
  function factory($table) {
    ...
    return $WGM_DAO_Factory->create($table);
  }
   ...
   *
   * symmetrical with getLink(), given either a WGM_DAO
  ...
  function addLink($col, $props) {
    ...
    if ($links && isset($links[$col]) ) {
      ...
      if (is_object($props) && 
          is_a('WGM_DAO') && $prop->getTableName() == $table ) {
        $obj = $props;
      } else {
  ...
  function update($dataObject = false) {
    if ( $this->_original && ($this->id != $this->_original->id) ) {
      WGM_Error::fatal('Object id does not match update cache. If you are trying to reuse this object for updates, call turnOffUpdateCache() first.');
    }
	    
    if ( !$this->canUpdate() ) {
      $className = $this->getClass();
      WGM_Error::fatal("This object cannot be updated: $className");
    }
  ...
  function __call($method,$params) {
    ...
    if (($type != 'set') && ($type != 'get')) {
      WGM_Error::fatal("method does not exist: $method ");
    }
