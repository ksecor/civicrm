...
require_once 'WGM/Action/Back.php';
require_once 'WGM/Action/Cancel.php';
require_once 'WGM/Action/Display.php';
require_once 'WGM/Action/Done.php';
require_once 'WGM/Action/Jump.php';
require_once 'WGM/Action/Next.php';
require_once 'WGM/Action/Process.php';
require_once 'WGM/Action/Refresh.php';
require_once 'WGM/Action/Submit.php';
require_once 'WGM/Action/Upload.php';

require_once 'WGM/StateMachine.php';
require_once 'WGM/Request.php';

class WGM_Controller extends HTML_QuickForm_Controller {
  ...
  /**
   * All WGM single or multi page pages should inherit from this class. This
  ...
  function addDefaultActions( $uploadDirectory = null, $uploadNames = null ) {
    $this->addAction('display', new WGM_Action_Display($this->_stateMachine));
    $this->addAction('next'   , new WGM_Action_Next   ($this->_stateMachine));
    $this->addAction('back'   , new WGM_Action_Back   ($this->_stateMachine));
    $this->addAction('process', new WGM_Action_Process($this->_stateMachine));
    $this->addAction('cancel' , new WGM_Action_Cancel ($this->_stateMachine));
    $this->addAction('refresh', new WGM_Action_Refresh($this->_stateMachine));
    $this->addAction('done'   , new WGM_Action_Done   ($this->_stateMachine));
    $this->addAction('jump'   , new WGM_Action_Jump   ($this->_stateMachine));
    $this->addAction('submit' , new WGM_Action_Submit ($this->_stateMachine));

    if ( ! empty( $uploadDirectory ) ) {
      $this->addAction('upload' , new WGM_Action_Upload ($this->_stateMachine,
                                                         $uploadDirectory,
							 $uploadNames));
    }
 ...
 function setStateMachine( $stateMachine) {
   ...
   *
   */
  function addPages( $stateMachine, $states = null ) {
    $stateNames = array( );
    ...
    foreach ( $states as $state ) {
      $className    = $state;
      $classString  = WGM_String::getLastTuple( $className );
      $stateNames[] = $classString;

      WGM_Utils::import( $className );
  ...
  function getButtonData( $buttonName ) {
    ...
    return WGM_Array::value( $buttonName, $buttonStore );
  }
