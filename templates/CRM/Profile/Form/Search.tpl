{if ! empty( $fields )}
 {if $groupId }
    <div id="id_{$groupId}_show" class="data-group">
       <a href="#" onclick="hide('id_{$groupId}_show'); show('id_{$groupId}'); return false;"><img src="{$config->resourceBase}i/TreePlus.gif" class="action-icon" alt="{ts}open section{/ts}"/></a><label>{ts}Search Criteria{/ts}</label><br />
    </div>
