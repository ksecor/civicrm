...
{else}
    ...
    <p>Each 'User Sharing Group' is presented as a separate form when new users register for an account, as well as when they edit an existing account.</p>{/ts}
    </div>
    ...
    {else}
       ...
       <img src="{$config->resourceBase}i/Inform.gif" alt="{ts}status{/ts}"> &nbsp;
         {capture assign=crmURL}{crmURL p='civicrm/admin/uf/group' q='action=add&reset=1'}{/capture}{ts 1=$crmURL}No User Sharing Groups have been created yet. You can <a href="%1">add one now</a>.{/ts}
       </div>
       {/if}
    {/if}
{/if}
