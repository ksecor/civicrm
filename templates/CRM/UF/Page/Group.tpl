...
{else}
    ...
    {capture assign=crmURL}{crmURL p='civicrm/admin/uf/group' q="action=preview"}{/capture}
    <p>{ts 1=$crmURL}Use the <a href="%1">Stand-alone Profile Form</a> link to get the HTML code needed to add a profile
    form to blocks or pages other than User Registration and My Account. You can add these 'stand-alone forms' to any website (e.g. for a signup form).{/ts}</p>
    </div>
