...
<div class="form-item">
    ...
    <dt>{$form.help_pre.label} {if $action == 2}{include file='CRM/Core/I18n/Dialog.tpl' table='civicrm_custom_group' field='help_pre' id=$gid}{/if}</dt><dd class="editor">{$form.help_pre.html}<br />
        <span class="description">{ts}Explanatory text displayed at the beginning of this group of fields.{/ts}</span></dd>
    <dt>{$form.help_post.label} {if $action == 2}{include file='CRM/Core/I18n/Dialog.tpl' table='civicrm_custom_group' field='help_post' id=$gid}{/if}</dt><dd class="editor">{$form.help_post.html}<br />
