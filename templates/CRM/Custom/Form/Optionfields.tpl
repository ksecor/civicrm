<fieldset><legend>{ts}Multiple Choice Options{/ts}</legend>
	...
	<tr id="optionField[{$index}]" class="form-item {cycle values="odd-row,even-row"}">
        <td> 
        {if $index GT 1}
            <a onclick="hide('optionField[{$index}]'); return false;" name="optionField[{$j}][hide]" href="#optionField[{$j}]" id="optionField[{$j}][hide]" class="form-link"><img src="{$config->resourceBase}i/TreeMinus.gif" class="action-icon" alt="{ts}hide field or section{/ts}"></a> 
