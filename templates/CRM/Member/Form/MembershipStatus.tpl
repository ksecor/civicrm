...
<fieldset><legend>{if $action eq 1}{ts}New Membership Status{/ts}{elseif $action eq 2}{ts}Edit Membership Status{/ts}{else}{ts}Delete Membership Status{/ts}{/if}</legend>
   ...
   {if $action eq 8}
    ...
    <dt>&nbsp;</dt><dd class="description html-adjust">{ts}Unit used for adjusting (add or substracting time) from the start event (e.g. day, month, year){/ts}</dd>
 	<dt>{$form.start_event_adjust_interval.label}</dt><dd class="html-adjust">{$form.start_event_adjust_interval.html}</dd>
    <dt>&nbsp;</dt><dd class="description html-adjust">{ts}This is the range of interval begins  from start event (signed integer). If the interval is zero then the range begins at start event.{/ts}</dd>
    ...
    <dt>&nbsp;</dt><dd class="description html-adjust">{ts}Unit used for adjusting (add or substracting time) from the ending event (e.g. day, month, year){/ts}</dd>
 	<dt>{$form.end_event_adjust_interval.label}</dt><dd class="html-adjust">{$form.end_event_adjust_interval.html}</dd>
    <dt>&nbsp;</dt><dd class="description html-adjust">{ts}This is the range of interval from end event (signed integer). If it is zero then range ends at start event.{/ts}</dd>
