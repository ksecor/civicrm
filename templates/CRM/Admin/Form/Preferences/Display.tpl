...
<fieldset><legend>{if $action eq 2 or $action eq 1}{ts}Site Preferences{/ts}{elseif $action eq 4}{ts}View Site Preferences{/ts}{/if}</legend>
      <table class="form-layout">
        ...
        <tr><td>&nbsp;</td><td class="description">{ts 1="http://wiki.civicrm.org/confluence//x/NCs" 2=$docURLTitle}Select the <strong>tabs</strong> that should be displayed when viewing a contact record. EXAMPLE: If your organization does not keep track of 'Relationships', then un-check this option to simplify the screen display. Tabs for Contributions, Memberships, and Events are also hidden if the corresponding component is not enabled (<a href='%1' target='_blank' title='%2'>read more...</a>).{/ts}</td></tr>
