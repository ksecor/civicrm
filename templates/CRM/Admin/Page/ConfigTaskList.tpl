...
<tr class="columnheader"><td colspan="2">{ts}Site configuration{/ts}</td></tr>
<tr class="even">
...
<table>
<tr class="columnheader"><td>{ts}Viewing and Editing Contacts{/ts}</td></tr>
...
<tr class="columnheader"><td>{ts}Sending Emails (includes contribution receipts and event confirmations){/ts}</td></tr>
<tr><td><ul class="tasklist">
...
<li><a href="{crmURL p="civicrm/admin/options/from_email_address" q="group=from_email_address&action=update&id=213&reset=1&destination=`$destination`"}" title="{$linkTitle}">{ts}From Email Addresses{/ts}</a> - {ts}Define general email address(es) that can be used as the FROM address when sending email to contacts from within CiviCRM (e.g. info@example.org){/ts}</li>
</ul>
</td></tr>

<tr class="columnheader"><td>{ts}Online Contributions / Online Membership Signup / Online Event Registration{/ts}</td></tr>
<tr><td><ul class="tasklist">
<li><a href="{crmURL p="civicrm/admin/paymentProcessor" q="action=add&reset=1&pp=PayPal&destination=`$destination`"}" title="{$linkTitle}">{ts}Payment Processors{/ts}</a> - {ts}Select and configure one or more payment processing services for online contributions, events and / or membership fees.{/ts}</li>
...
<table>
<tr class="columnheader"><td>{ts}Organize your contacts{/ts}</td></tr>
<tr><td><ul class="tasklist">
...
<tr class="columnheader"><td>{ts}Customize Data, Forms and Screens{/ts}</td></tr>
<tr><td><ul class="tasklist">
...
<table>
<tr class="columnheader"><td>{ts}Components{/ts}</td></tr>
<tr><td>
<ul class="tasklist">
