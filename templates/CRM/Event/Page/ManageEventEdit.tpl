...
<tr>
    ...
    <td>{ts}Set event title, type (conference, concert etc.), description, start and end dates, and activate the event.{/ts}</td>
</tr>
<tr>
...
<tr>
    <td class="nowrap"><a href="{crmURL p='civicrm/admin/event' q="reset=1&action=update&id=`$id`&subPage=Fee"}" id="idFee">&raquo; {ts}Event Fees{/ts}</a></td>
...
<tr>
    <td class="nowrap"><a href="{crmURL p='civicrm/admin/event' q="reset=1&action=update&id=`$id`&subPage=Registration"}" id="idRegistration">&raquo; {ts}Online Registration{/ts}</a></td>
