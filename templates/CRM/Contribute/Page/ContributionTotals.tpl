    ...
    {if $contributionSummary }
     <table class="form-layout-compressed">
      ...
      {if $contributionSummary.total.amount}
        <th style="border-left: 1px solid #5A8FDB;">{ts}Total Amount{/ts} - {$contributionSummary.total.amount}</th>
        <th class="right" width="39px"> &nbsp; </th>
        <th class="right"> &nbsp; {ts}# Contributions{/ts} - {$contributionSummary.total.count}</th>
        <th class="right" style="border-right: 1px solid #5A8FDB;"> &nbsp; {ts}Avg Amount{/ts} - {$contributionSummary.total.avg}</th>
      {/if}
