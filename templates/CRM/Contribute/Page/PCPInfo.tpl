...
<div class="messages status">
  <dl>
	...
	<dd><p>This is your Personal Campaign Page (PCP). It belongs to <a href="{crmURL p='civicrm/contribute/manage' q="reset=1&action=update&id="}{$owner.id}"><strong>&raquo; {ts}Contribution Page{/ts}</strong></a>{if $owner.start_date}, which is active from <strong>{$owner.start_date|truncate:10:''|crmDate}</strong> until <strong>{$owner.end_date|truncate:10:''|crmDate}</strong>. Current status of your PCP is: <strong {if $owner.status ne 'Approved' }class=disabled {/if}>{$owner.status}</strong>{/if}.</p><p><strong>You can perform following actions on your page:</strong></p></dd>
