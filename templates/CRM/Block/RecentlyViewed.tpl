...
<div id="recently-viewed">
    ...
    {foreach from=$recentlyViewed item=item}
         ...
         {if $item.image_url}
            <div class="icon {if $item.subtype}{$item.subtype}{else}{$item.type}{/if}-icon" style="background: url('{$item.image_url}')"></div>
         {else}
            <div class="icon {$item.type}{if $item.subtype}-subtype{/if}-icon"></div>
         {/if}
