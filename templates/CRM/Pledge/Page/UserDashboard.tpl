...
<table class="selector">
  ...
  <tr id='rowid{$row.pledge_id}' class="{cycle values="odd-row,even-row"} {if $row.pledge_status_id eq 'Overdue' } disabled{/if}">
    ...
    <td>{$row.action}<br/>{if $row.pledge_contribution_page_id and ($row.pledge_status_id neq 'Completed')}<a href="{crmURL p='civicrm/contribute/transact' q="reset=1&id=`$row.pledge_contribution_page_id`&pid=`$row.pledge_id`"}">{ts}Pay Online{/ts}</a><br/>{/if}
