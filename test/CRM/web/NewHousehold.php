...
class TestOfNewHouseholdForm extends WebTestCase 
{
    function setUp( ) 
    {
    }
    ...
    function testNewHousehold()
    {
        ...
        if ($this->assertLink('New Household')) {
            $this->clickLink('New Household');
        }
        
        $this->assertResponse(200);
        
        // Household Block.
        $this->assertWantedText("Household");
        $household_name = 'AAB Household.';
        $nick_name      = 'AAB members.';
        $this->setFieldById('household_name', $household_name);
        ...
        $note = 'Note for AAB Household. We beleve in ... \'what u can dream, we can build it.\'';
        $this->setFieldbyId('note', $note);
        
        $this->clickSubmitByName('_qf_Edit_next_view');
        
        $this->assertResponse(200);
        $this->assertWantedText("Your Household contact record has been saved.");
    }
}
?>