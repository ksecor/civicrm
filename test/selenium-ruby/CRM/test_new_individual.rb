...
class TC_TestNewIndividual < Test::Unit::TestCase
  ...
  def add_new_individual
    ...
    if @selenium.is_text_present("Your Individual contact record has been saved.")
      assert @selenium.is_text_present("Your Individual contact record has been saved.")
    else
      assert @selenium.is_text_present("One matching contact was found. You can edit it here: Abhilasha Vijay Vasu, or click Save Duplicate Contact button below.")
    end
  end
  ...
  def save_and_new_individual
    ...
    @selenium.select "location[1][phone][1][phone_type]", "label=Mobile"
    @selenium.type "location[1][phone][1][phone]", "09867564649"
    @selenium.type "document.Edit.elements['location[1][email][1][email]']", "vivek@webaccess.co.in"
    @selenium.type "location[1][im][1][name]", "MSN"
    @selenium.type "location[1][address][street_address]", "S 116W Northpoint Blvd SE"
    @selenium.type "location[1][address][supplemental_address_1]", "Editorial Dept"
    @selenium.type "location[1][address][supplemental_address_2]", "andheri -E"
    ...
    if @selenium.is_text_present("Your Individual contact record has been saved.")
      ...
      assert @selenium.is_text_present("One matching contact was found. You can edit it here: Vivek Vijay Vasu, or click Save Duplicate Contact button below.")
    end
  end
  ...
  def search_individual
    ...
    @selenium.type "document.Search.sort_name", "Vivek Vasu"
    
    #click search     
    @page.click_and_wait "document.Search._qf_Search_refresh"
    
    #click name
    assert @selenium.is_text_present("Vasu, Vivek")
    assert_equal "Vasu, Vivek", @selenium.get_text("link=Vasu, Vivek")
    @page.click_and_wait "link=Vasu, Vivek"
  end
end
