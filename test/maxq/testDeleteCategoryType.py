...
class testDeleteCategoryType(PyHttpTestCase):
    def runTest(self):
        ...
        params = [
            ('''action''', '''delete'''),
            ('''id''', '''7'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/category?action=delete&id=7''') % drupal_path)
        url = "%s/admin/contact/category" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
...
if __name__ == 'main':
    test = testDeleteCategoryType("testDeleteCategoryType")
    test.Run()
