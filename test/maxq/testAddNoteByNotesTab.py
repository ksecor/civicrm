...
class testAddNoteByNotesTab(PyHttpTestCase):
    def runTest(self):
        ...
        params = [
            ('''cid''', '''70'''),
            ('''action''', '''add'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/civicrm/contact/view/note?cid=70&action=add''') % drupal_path)
        url = "%s/civicrm/contact/view/note" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
...
if __name__ == 'main':
    test = testAddNoteByNotesTab("testAddNoteByNotesTab")
    test.Run()
