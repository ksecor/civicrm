...
class testAdvSearchByAllCriteria(PyHttpTestCase):
    def runTest(self):
        ...
        params = [
            ...
            ('''postal_code_low''', '''10'''),
            ('''postal_code_high''', '''20'''),
            ('''cb_location_type[1]''', '''1'''),
            ('''_qf_Search_refresh''', '''Search'''),
            ('''crmPID''', '''1'''),
            ('''crmPID_B''', '''1'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/civicrm/contact/search/advanced?_qf_default=Search:refresh&cb_group[2]=1&cb_category[4]=1&cb_category[5]=1&sort_name=a&street_name=y&city=u&state_province=1000&country=1228&postal_code=&postal_code_low=10&postal_code_high=20&cb_location_type[1]=1&_qf_Search_refresh=Search&crmPID=1&crmPID_B=1''') % drupal_path)
        url = "%s/civicrm/contact/search/advanced" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
        ...
        self.msg("Testing URL: %s" % self.replaceURL('''%s/drupal/crm/css/civicrm.css''') % drupal_path)
        url = "%s/crm/css/civicrm.css" % drupal_path
        params = None
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
        ...
        self.msg("Testing URL: %s" % self.replaceURL('''%s/drupal/crm/js/Common.js''') % drupal_path)
        url = "%s/crm/js/Common.js" % drupal_path
        params = None
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
