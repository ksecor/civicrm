...
class testDisableLocationType(PyHttpTestCase):
    def runTest(self):
        ...
        params = [
            ('''action''', '''disable'''),
            ('''id''', '''6'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/locationType?action=disable&id=6''') % drupal_path)
        url = "%s/admin/contact/locationType" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
...
if __name__ == 'main':
    test = testDisableLocationType("testDisableLocationType")
    test.Run()
