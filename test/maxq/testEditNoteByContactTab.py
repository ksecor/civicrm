...
class testEditNoteByContactTab(PyHttpTestCase):
    def runTest(self):
        ...
        params = [
            ('''nid''', '''202'''),
            ('''action''', '''update'''),]
        #self.msg("Testing URL: %s" % self.replaceURL('''%s/civicrm/contact/view/note?nid=202&action=update''') % drupal_path)
        ...
        params = [
            ...
            ('''note''', '''Test Note from Contact tab...Doing test for Editing the Note'''),
            ('''_qf_Note_next''', '''Save'''),]
        #self.msg("Testing URL: %s" % self.replaceURL('''%s/civicrm/contact/view/note?_qf_default=Note:next&note=Test Note from Contact tab...Doing test for Editing the Note&_qf_Note_next=Save''') % drupal_path)
        url = "%s/civicrm/contact/view/note" % drupal_path
        self.msg("Testing URL: %s" % url)
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
