...
class testEditNoteByNotesTab(PyHttpTestCase):
    def runTest(self):
        ...
        params = [
            ('''nid''', '''199'''),
            ('''action''', '''update'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/civicrm/contact/view/note?nid=199&action=update''') % drupal_path)
        url = "%s/civicrm/contact/view/note" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
        ...
        params = [
            ('''note''', '''New Note for this contact
Checking it with the maxQ test case
Editing the Note and checking with the maxQ test case
'''),
            ('''_qf_Note_next''', '''Save'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/civicrm/contact/view/note?note=New Note for this contact
Checking it with the maxQ test case
Editing the Note and checking with the maxQ test case
&_qf_Note_next=Save''') % drupal_path)
        url = "%s/civicrm/contact/view/note" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
...
if __name__ == 'main':
    test = testEditNoteByNotesTab("testEditNoteByNotesTab")
    test.Run()
