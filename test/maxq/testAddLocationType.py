...
class testAddLocationType(PyHttpTestCase):
    def runTest(self):
        ...
        params = [
            ('''action''', '''add'''),
            ('''reset''', '''1'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/locationType?action=add&reset=1''') % drupal_path)
        url = "%s/admin/contact/locationType" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
        ...
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/locationType?name=Field&description=Site Location&_qf_LocationType_next=Save''') % drupal_path)
        url = "%s/admin/contact/locationType" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
