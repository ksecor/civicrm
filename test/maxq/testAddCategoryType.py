...
class testAddCategoryType(PyHttpTestCase):
    def runTest(self):
        ...
        params = [
            ('''action''', '''add'''),
            ('''reset''', '''1'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/category?action=add&reset=1''') % drupal_path)
        url = "%s/admin/contact/category" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
        ...
        params = [
            ('''name''', '''Pvt. Ltd.'''),
            ('''description''', '''Private Limited Entity'''),
            ('''_qf_Category_next''', '''Save'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/category?name=Pvt. Ltd.&description=Private Limited Entity&_qf_Category_next=Save''') % drupal_path)
        url = "%s/admin/contact/category" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
...
if __name__ == 'main':
    test = testAddCategoryType("testAddCategoryType")
    test.Run()
