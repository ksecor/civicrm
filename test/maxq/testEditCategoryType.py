...
class testEditCategoryType(PyHttpTestCase):
    def runTest(self):
        ...
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/category''') % drupal_path)
        url = "%s/admin/contact/category" % drupal_path
        params = None
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
        ...
        params = [
            ('''action''', '''update'''),
            ('''id''', '''7'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/category?action=update&id=7''') % drupal_path)
        url = "%s/admin/contact/category" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
        ...
        params = [
            ('''name''', '''Public Ltd.'''),
            ('''description''', '''Public Limited Entity'''),
            ('''_qf_Category_next''', '''Save'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/category?name=Public Ltd.&description=Public Limited Entity&_qf_Category_next=Save''') % drupal_path)
        url = "%s/admin/contact/category" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
        ...
        params = [
            ('''reset''', '''1'''),
            ('''action''', '''browse'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/admin/contact/category?reset=1&action=browse''') % drupal_path)
        url = "%s/admin/contact/category" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
...
if __name__ == 'main':
    test = testEditCategoryType("testEditCategoryType")
    test.Run()
