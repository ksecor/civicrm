...
class testAddUpdateTags(PyHttpTestCase):
    def runTest(self):
        ...
        self.msg("Testing URL: %s" % self.replaceURL('''%s/civicrm/contact/view/tag?categoryList[3]=1&categoryList[5]=1&_qf_Tag_next=Update Tags''') % drupal_path)
        url = "%s/civicrm/contact/view/tag" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
        ...
        params = [
            ('''action''', '''browse'''),]
        self.msg("Testing URL: %s" % self.replaceURL('''%s/drupal/civicrm/contact/view/tag?action=browse''') % drupal_path)
        url = "%s/civicrm/contact/view/tag" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
...
if __name__ == 'main':
    test = testAddUpdateTags("testAddUpdateTags")
    test.Run()
