...
class testViewNoteByNoteTab(PyHttpTestCase):
    def runTest(self):
        ...
        params = [
            ('''nid''', '''44'''),
            ('''action''', '''view'''),]
        #self.msg("Testing URL: %s" % self.replaceURL('''%s/civicrm/contact/view/note?nid=44&action=view''') % drupal_path)
        url = "%s/civicrm/contact/view/note" % drupal_path
        self.msg("Testing URL: %s" % url)
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
...
if __name__ == 'main':
    test = testViewNoteByNoteTab("testViewNoteByNoteTab")
    test.Run()
