...
class testSearchByONameOraganization(PyHttpTestCase):
    def runTest(self):
        ...
        self.msg("Testing URL: %s" % self.replaceURL('''%s/civicrm/contact/search?_qf_default=Search:refresh&contact_type=Organization&group=any&category=any&sort_name=Zope&_qf_Search_refresh=Search''') % drupal_path)
        url = "%s/civicrm/contact/search" % drupal_path
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
        self.msg("Response code: %s" % self.getResponseCode())
