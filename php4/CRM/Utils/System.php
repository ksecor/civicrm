...
$GLOBALS['_CRM_UTILS_SYSTEM']['script'] =  'index.php';
...
class CRM_Utils_System {
     ...
     */
     function makeURL( $urlVar ) {
        return CRM_Utils_System::url( $_GET['q'], CRM_Utils_System::getLinksUrl( $urlVar ) );
    }
    ...
    function url($path = null, $query = null, $absolute = true, $fragment = null ) {
        ...
        if (! $config->cleanURL ) {
            if ( isset( $path ) ) {
                if ( isset( $query ) ) {
                    return $base . $GLOBALS['_CRM_UTILS_SYSTEM']['script'] .'?q='. $path .'&amp;'. $query . $fragment;
                } else {
                    return $base . $GLOBALS['_CRM_UTILS_SYSTEM']['script'] .'?q='. $path . $fragment;
                }
            } else {
                if ( isset( $query ) ) {
                    return $base . $GLOBALS['_CRM_UTILS_SYSTEM']['script'] .'?'. $query . $fragment;
                } else {
     ...
     * @static
     */
     function classNameToFile($className, $phpFile=true) {
        ...
        if (CRM_Utils_System::isPHP4()) {
            foreach ($classNameComponent as $k => $v) {
                if (array_key_exists($v, $GLOBALS['_CRM_UTILS_SYSTEM']['php4SpecialClassName'])) {
                    $classNameComponent[$k] = $GLOBALS['_CRM_UTILS_SYSTEM']['php4SpecialClassName'][$v]; // special case hence replace
                } else {
                    $classNameComponent[$k] = ucfirst($v); // regular component so just upcase first character
                }
            }
        }
