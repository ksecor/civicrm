...
class CRM_Contact_Page_GroupContact {
     ...
     * @access public
     */
     function edit( $page, $mode, $groupId = null ) {
     ...
     function run( $page ) {
        ...
        if ( $action == CRM_CORE_ACTION_DELETE ) {
            ...
            if (is_numeric($groupContactId) && strlen(trim($status))) {
                CRM_Contact_Page_GroupContact::del( $groupContactId,$status );
            }
        }

        CRM_Contact_Page_GroupContact::edit( $page, CRM_CORE_ACTION_ADD );
        CRM_Contact_Page_GroupContact::browse( $page );
    }
