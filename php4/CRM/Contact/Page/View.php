...
define( 'CRM_CONTACT_PAGE_VIEW_MODE_NONE',0);
define( 'CRM_CONTACT_PAGE_VIEW_MODE_NOTE',1);
define( 'CRM_CONTACT_PAGE_VIEW_MODE_GROUP',2);
define( 'CRM_CONTACT_PAGE_VIEW_MODE_REL',4);
define( 'CRM_CONTACT_PAGE_VIEW_MODE_TAG',8);
define( 'CRM_CONTACT_PAGE_VIEW_MODE_CD',16);
define( 'CRM_CONTACT_PAGE_VIEW_MODE_ACTIVITY',32);
...
require_once 'CRM/Contact/Page/Note.php';
require_once 'CRM/Contact/Page/Relationship.php';
require_once 'CRM/Contact/Page/Tag.php';
require_once 'CRM/Contact/Page/GroupContact.php';
require_once 'CRM/Contact/Page/CustomData.php';
require_once 'CRM/Contact/Page/Activity.php';
...
class CRM_Contact_Page_View extends CRM_Core_Page {
    ...
    function run()
    {
        ...
        if (isset($prevMode) && $prevMode != $this->_mode) {
            $this->set('action', CRM_CORE_ACTION_BROWSE);
        }
        $this->set('mode', $this->_mode);

        // call the appropriate run method for other detailed page views.
        if ( $this->_mode == CRM_CONTACT_PAGE_VIEW_MODE_NONE) {
            $this->runModeNone( );
        } else if ( $this->_mode == CRM_CONTACT_PAGE_VIEW_MODE_NOTE) {
            CRM_Contact_Page_Note::run( $this );
        } else if ( $this->_mode == CRM_CONTACT_PAGE_VIEW_MODE_REL) {
            CRM_Contact_Page_Relationship::run( $this );
        } else if ( $this->_mode == CRM_CONTACT_PAGE_VIEW_MODE_TAG) {
            CRM_Contact_Page_Tag::run( $this );
        } else if ( $this->_mode == CRM_CONTACT_PAGE_VIEW_MODE_GROUP) {
            CRM_Contact_Page_GroupContact::run( $this );
        } else if ( $this->_mode == CRM_CONTACT_PAGE_VIEW_MODE_CD) {
            CRM_Contact_Page_CustomData::run( $this );
        } else if ( $this->_mode == CRM_CONTACT_PAGE_VIEW_MODE_ACTIVITY) {
    ...
    function getContactDetails()
    {
        ...
        if ( isset( $displayName ) && $this->_mode != CRM_CONTACT_PAGE_VIEW_MODE_NONE) {
