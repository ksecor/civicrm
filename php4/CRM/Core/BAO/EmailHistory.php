...
class CRM_Core_BAO_EmailHistory extends CRM_Core_DAO_EmailHistory {
     ...
     * @static
     */
     function sendEmail( &$contactIds, &$subject, &$message ) {
        ...
        foreach ( $contactIds as $contactId ) {
            if ( CRM_Core_BAO_EmailHistory::sendMessage( $from, $contactId, $subject, $message, $email->id ) ) {
                $sent++;
            } else {
                $notSent++;
            }
     ...
     * @static
     */
     function sendMessage( $from, $toID, &$subject, &$message, $activityID ) {
