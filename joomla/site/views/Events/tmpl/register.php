...
<div class="vevent">
            ...
            <tr><td><label>JText::_('Contact')</label></td>
            	<td>	/* loop on any phones and emails for this event */
               		<?php foreach from=$location.1.phone item=phone ?>
                		<?php if $phone.phone ?>
               		     		<?php if $phone.phone_type}{$phone.phone_type_display}:<?php endif; ?> 
    ...
    <?php if $is_online_registration AND $context NEQ 'dashboard' ?>
        <div class="action-link">
            <strong><a href="<?php echo this-> $registerURL ?>" title="<?php echo this->$registerText ?>"><?php echo this->$registerText ?></a></strong>
        </div>
    <?php endif; ?>
	</div>
     <?php ifif $event.is_public  ?>
      <div class="action-link">
	  //need to do
