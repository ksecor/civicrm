...
{assign var='first' value=false}
     ...
     {$field.name} {$field.sqlType} {if $field.required}NOT NULL{/if} {if $field.autoincrement}AUTO_INCREMENT{/if} {if $field.comment}COMMENT '{$field.comment}'{/if}
...
{if $table.UNIQUE} UNIQUE{/if} INDEX {$index.name}({$index.keys})
{/foreach} {* table.index *}
{/if} {* table.index *}
