...
INSERT INTO civicrm_uf_field
       ...
       (18, 6,           'email',                 1,           0,           3,      'User and User Admin Only',  0,           0,             NULL,             '{ts escape="sql"}Email Address{/ts}',         'Contact',     NULL);
...
( NULL,								'{ts escape="sql"}Full-text Search{/ts}','{ts escape="sql"}Full-text Search{/ts}','',			NULL,		'',			'4',	'1',		NULL ),
( 'civicrm/contact/search/builder',				'{ts escape="sql"}Search Builder{/ts}',	'{ts escape="sql"}Search Builder{/ts}',	'' , 			NULL ,		'' , 			'4',	'1' ,		'1' ),
...
( NULL,								'{ts escape="sql"}Custom Searches...{/ts}','{ts escape="sql"}Custom Searches...{/ts}' ,'' , 		NULL ,		'' , 			'4',	'1' ,		NULL ),
( 'civicrm/contact/search/custom?reset=1&amp;csid=11', 		'{ts escape="sql"}Contacts by Date Added{/ts}', '{ts escape="sql"}Contacts by Date Added{/ts}' , '' , 	NULL , 		'' , 			'15',	'1' ,		NULL ),
( 'civicrm/contact/search/custom?reset=1&amp;csid=6',		'{ts escape="sql"}Proximity Search{/ts}','{ts escape="sql"}Proximity Search{/ts}' , '' , 		NULL ,		'' , 			'15',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Contacts{/ts}',	'{ts escape="sql"}Contacts{/ts}' ,	'', 			NULL ,		'' , 			NULL,	'1' ,		NULL ),
( 'civicrm/contact/add&amp;ct=Individual', 			'{ts escape="sql"}New Individual{/ts}',	'{ts escape="sql"}New Individual{/ts}',	'' , 			NULL ,		'' , 			'18',	'1' ,		NULL ),
( 'civicrm/contact/add&amp;ct=Household',			'{ts escape="sql"}New Household{/ts}',	'{ts escape="sql"}New Household{/ts}',	'' , 			NULL ,		'' , 			'18',	'1' ,		NULL ),
( 'civicrm/contact/add&amp;ct=Organization',			'{ts escape="sql"}New Organization{/ts}','{ts escape="sql"}New Organization{/ts}' ,'' , 		NULL ,		'' ,			'18',	'1' ,		NULL ),
( 'civicrm/activity&amp;action=add&amp;context=standalone', 	'{ts escape="sql"}New Activity{/ts}', 	'{ts escape="sql"}New Activity{/ts}' ,	'' , 			NULL,		'' , 			'18',	'1' ,		'1' ),
( 'civicrm/import/contact',					'{ts escape="sql"}Import Contacts{/ts}','{ts escape="sql"}Import Contacts{/ts}','' , 			NULL,		'' , 			'18',	'1' ,		NULL ),
( 'civicrm/import/activity',					'{ts escape="sql"}Import Activities{/ts}','{ts escape="sql"}Import Activities{/ts}' ,'' , 		NULL,		'' , 			'18',	'1' ,		'1' ),
( 'civicrm/group/add',						'{ts escape="sql"}New Group{/ts}',	'{ts escape="sql"}New Group{/ts}' ,	'' , 			NULL,		'' , 			'18',	'1' ,		NULL ),
( 'civicrm/group',						'{ts escape="sql"}Manage Groups{/ts}',	'{ts escape="sql"}Manage Groups{/ts}' ,	'' , 			NULL,		'' , 			'18',	'1' ,		'1' ),
( 'civicrm/admin/tag&amp;action=add',				'{ts escape="sql"}New Tag{/ts}', 	'{ts escape="sql"}New Tag{/ts}' ,	'' , 			NULL,		'' , 			'18',	'1' ,		NULL ),
( 'civicrm/admin/tag', 						'{ts escape="sql"}Manage Tags (Categories){/ts}','{ts escape="sql"}Manage Tags (Categories){/ts}' ,'' , NULL,		'' , 			'18',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Contributions{/ts}', 	'{ts escape="sql"}Contributions{/ts}' ,	'' , 			'access CiviContribute' ,'' ,		NULL,	'1' ,		NULL ),
( 'civicrm/contribute', 					'{ts escape="sql"}Dashboard{/ts}', 	'{ts escape="sql"}CiviContribute Dashboard{/ts}' ,'' , 		NULL,		'' , 			'29',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}New Contribution{/ts}','{ts escape="sql"}New Contribution{/ts}' ,'' , 		NULL,		'' , 			'29',	'1' ,		NULL ),
( 'civicrm/contribute/search', 					'{ts escape="sql"}Find Contributions{/ts}','{ts escape="sql"}Find Contributions{/ts}' ,'' , 		NULL,		'' , 			'29',	'1' ,		NULL ),
( 'civicrm/contribute/import',					'{ts escape="sql"}Import Contributions{/ts}','{ts escape="sql"}Import Contributions{/ts}' ,'' , 	NULL,		'' , 			'29',	'1' ,		'1' ),
( 'civicrm/admin/contribute&amp;action=add',			'{ts escape="sql"}New Contribution Page{/ts}','{ts escape="sql"}New Contribution Page{/ts}' ,'' , 	NULL,		'' , 			'29',	'1' ,		NULL ),
( 'civicrm/admin/contribute', 					'{ts escape="sql"}Manage Contribution Pages{/ts}','{ts escape="sql"}Manage Contribution Pages{/ts}' ,'' , NULL,		'' , 			'29',	'1' ,		'1' ),
( 'civicrm/admin/pcp',						'{ts escape="sql"}Manage Personal Campaign Pages{/ts}','{ts escape="sql"}Personal Campaign Pages{/ts}' ,'' , NULL,	'' , 			'29',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Events{/ts}',		'{ts escape="sql"}Events{/ts}' ,	'' , 			'access CiviEvent' ,'' , 		NULL,	'1' ,		NULL ),
( 'civicrm/event', 						'{ts escape="sql"}Dashboard{/ts}',	'{ts escape="sql"}CiviEvent Dashboard{/ts}' ,'' , 		NULL,		'' , 			'37',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Register New Participant{/ts}','{ts escape="sql"}Register New Participant{/ts}' ,'' , NULL,		'' , 			'37',	'1' ,		NULL ),
( 'civicrm/event/search', 					'{ts escape="sql"}Find Participants{/ts}','{ts escape="sql"}Find Participants{/ts}' ,'' , 		NULL,		'' , 			'37',	'1' ,		NULL ),
( 'civicrm/event/import', 					'{ts escape="sql"}Import Participants{/ts}','{ts escape="sql"}Import Participants{/ts}' ,'' , 		NULL,		'' , 			'37',	'1' ,		'1' ),
( 'civicrm/event/add&amp;action=add',				'{ts escape="sql"}New Event{/ts}',	'{ts escape="sql"}New Event{/ts}' ,	'' , 			NULL,		'' , 			'37',	'1' ,		NULL ),
( 'civicrm/event/manage', 					'{ts escape="sql"}Manage Events{/ts}',	'{ts escape="sql"}Manage Events{/ts}' ,	'' , 			NULL,		'' , 			'37',	'1' ,		NULL ),
( 'civicrm/admin/price&amp;action=add',				'{ts escape="sql"}New Price Set{/ts}',	'{ts escape="sql"}New Price Set{/ts}' ,'' , 			NULL,		'' , 			'37',	'1' ,		NULL ),
( 'civicrm/event/price', 					'{ts escape="sql"}Manage Price Sets{/ts}','{ts escape="sql"}Manage Price Sets{/ts}' ,'' , 		NULL,		'' , 			'37',	'1',		NULL ),
( NULL,								'{ts escape="sql"}Mailings{/ts}',	'{ts escape="sql"}Mailings{/ts}' ,	'' , 			'access CiviMail' ,'' ,			 NULL,	'1' ,		NULL ),
( 'civicrm/mailing/send',					'{ts escape="sql"}New Mailing{/ts}',	'{ts escape="sql"}New Mailing{/ts}' ,	'' , 			NULL,		'' , 			'46',	'1' ,		NULL ),
( 'civicrm/mailing/browse/unscheduled&amp;scheduled=false', 	'{ts escape="sql"}Draft and Unscheduled Mailings{/ts}','{ts escape="sql"}Draft and Unscheduled Mailings{/ts}' ,'' , NULL ,'' , 			'46',	'1' ,		NULL ),
( 'civicrm/mailing/browse/scheduled&amp;scheduled=true', 	'{ts escape="sql"}Scheduled and Sent Mailings{/ts}','{ts escape="sql"}Scheduled and Sent Mailings{/ts}' ,'' , NULL ,	'' , 			'46',	'1' ,		NULL ),
( 'civicrm/mailing/browse/archived', 				'{ts escape="sql"}Archived Mailings{/ts}','{ts escape="sql"}Archived Mailings{/ts}' ,'' , 		NULL ,		'' , 			'46',	'1' ,		NULL ),
( 'civicrm/admin/component',					'{ts escape="sql"}Headers, Footers, and Automated Messages{/ts}','{ts escape="sql"}Headers, Footers, and Automated Messages{/ts}' ,'' , NULL ,'' , '46',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Memberships{/ts}',	'{ts escape="sql"}Memberships{/ts}' ,	'' , 			'access CiviMember' ,'' , 		NULL,	'1' ,		NULL ),
( 'civicrm/member', 						'{ts escape="sql"}Dashboard{/ts}',	'{ts escape="sql"}CiviMember Dashboard{/ts}' ,'' , 		NULL,		'' , 			'52',	'1' ,		NULL ), 
( NULL,								'{ts escape="sql"}New Member{/ts}',	'{ts escape="sql"}New Member{/ts}' ,'' , 			NULL,		'' , 			'52',	'1' ,		NULL ),
( 'civicrm/member/search', 					'{ts escape="sql"}Find Members{/ts}',	'{ts escape="sql"}Find Members{/ts}' ,'' , 			NULL,		'' , 			'52',	'1' ,		NULL ),
( 'civicrm/member/import', 					'{ts escape="sql"}Import Members{/ts}',	'{ts escape="sql"}Import Members{/ts}' ,'' , 			NULL,		'' , 			'52',	'1' ,		NULL ),
...
( 'civicrm/grant', 						'{ts escape="sql"}New Grant{/ts}',	'{ts escape="sql"}New Grant{/ts}' ,	'' , 			NULL,		'' , 			'58',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Cases{/ts}',		'{ts escape="sql"}Cases{/ts}' ,		'' , 			'access CiviCase' ,'' , 		'57',	'1' ,		NULL ),
( 'civicrm/case',						'{ts escape="sql"}Dashboard{/ts}',	'{ts escape="sql"}CiviCase Dashboard{/ts}' ,'' , 		NULL,		'' , 			'60',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}New Case for New Client{/ts}','{ts escape="sql"}New Case for New Client{/ts}' ,'' ,	NULL,		'' , 			'60',	'1' ,		NULL ),
( 'civicrm/case/search',					'{ts escape="sql"}Find Cases{/ts}',	'{ts escape="sql"}Find Cases{/ts}' ,	'' , 			NULL,		'' , 			'60',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Administer{/ts}',	'{ts escape="sql"}Administer{/ts}' ,	'' , 			NULL,		'' , 			NULL,	'1' ,		NULL ),
( 'civicrm/admin', 						'{ts escape="sql"}Administration Console{/ts}','{ts escape="sql"}Administer CiviCRM{/ts}' ,'' , 	NULL,		'' , 			'64',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Customize{/ts}',	'{ts escape="sql"}Customize{/ts}' ,	'' , 			NULL,		'' , 			'64',	'1' ,		NULL ),
( 'civicrm/admin/custom/group',					'{ts escape="sql"}Custom Data{/ts}',	'{ts escape="sql"}Custom Data{/ts}' ,	'' , 			NULL,		'' ,			'66',	'1' ,		NULL ),
( 'civicrm/admin/uf/group',					'{ts escape="sql"}CiviCRM Profile{/ts}','{ts escape="sql"}CiviCRM Profile{/ts}','' , 			NULL,		'' , 			'66',	'1' ,		NULL ),
( 'civicrm/admin/options/custom_search&group=custom_search',	'{ts escape="sql"}Manage Custom Searches{/ts}','{ts escape="sql"}Manage Custom Searches{/ts}' ,'' , 	NULL,		'' , 			'66',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Configure{/ts}',	'{ts escape="sql"}Configure{/ts}' ,	'' , 			NULL,		'' , 			'64',	'1' ,		NULL ),
( 'civicrm/admin/configtask', 					'{ts escape="sql"}Configuration Checklist{/ts}','{ts escape="sql"}Configuration Checklist{/ts}' ,'' , 	NULL,		'' , 			'70',	'1' ,		NULL ),
( 'civicrm/admin/setting',					'{ts escape="sql"}Global Settings{/ts}','{ts escape="sql"}Global Settings{/ts}','' , 			NULL,		'' , 			'70',	'1' ,		NULL ),
...
( 'civicrm/admin/mapping',					'{ts escape="sql"}Import/Export Mappings{/ts}','{ts escape="sql"}Import/Export Mappings{/ts}' ,'' , 	NULL,		'' , 			'70',	'1' ,		NULL ),
( 'civicrm/admin/messageTemplates', 				'{ts escape="sql"}Message Templates{/ts}','{ts escape="sql"}Message Templates{/ts}' ,'' , 		NULL,		'' , 			'70',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Manage{/ts}',		'{ts escape="sql"}Manage{/ts}' ,	'' ,			NULL,		'' , 			'64',	'1' ,		NULL ),
( 'civicrm/admin/synchUser',					'{ts escape="sql"}Synchronize Users to Contacts{/ts}','{ts escape="sql"}Synchronize Users to Contacts{/ts}' ,'' , NULL,	'' , 			'76',	'1' ,		NULL ),
( 'civicrm/admin/access', 					'{ts escape="sql"}Access Control{/ts}',	'{ts escape="sql"}Access Control{/ts}',	'' , 			NULL,		'' , 			'76',	'1' ,		NULL ),
( NULL,								'{ts escape="sql"}Option Lists{/ts}',	'{ts escape="sql"}Option Lists{/ts}',	'' , 			NULL,		'' , 			'64',	'1' ,		NULL ),
( 'civicrm/admin/options/activity_type&amp;group=activity_type','{ts escape="sql"}Activity Types{/ts}',	'{ts escape="sql"}Activity Types{/ts}',	'' , 			NULL,		'' , 			'79',	'1' ,		NULL ),
( 'civicrm/admin/options/gender&amp;group=gender', 		'{ts escape="sql"}Gender Options{/ts}',	'{ts escape="sql"}Gender Options{/ts}',	'' , 			NULL,		'' , 			'79',	'1' ,		NULL ),
