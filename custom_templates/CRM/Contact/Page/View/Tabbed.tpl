...
{else}
<div id="mainTabContainer" dojoType="TabContainer" style="width: 100%; height: 1600px; overflow-y: auto;" selectedTab="{$selectedChild}">
...
*}
    ...
    {if $relationship.data and $contact_type eq 'Individual'}
    <tr><td><label>{ts}Organisation(s){/ts}:</label></td><td>
        {foreach from=$relationship.data key=dontCare item=rel}
           {if $rel.civicrm_relationship_type_id eq 4 and $rel.rtype eq "a_b" }
               <a href="{crmURL p='civicrm/contact/view' q="action=view&reset=1&cid=`$rel.cid`"}">{$rel.name}</a><br>
           {/if}
        {/foreach}
    </td></tr>
    {/if}
...
{assign var="pr_int_ok" value="1"}
{foreach from=$privacy item=privacy_val key=privacy_label}
   {if $privacy_val eq 1}{assign var="pr_int_ok" value="1"} {/if}
{/foreach}
{if $preferred_communication_method_display or $preferred_mail_format_display neq 'Both' or $is_opt_out or $pr_int_ok}
...
{/if}
{if $preferred_mail_format_display neq 'Both'}
  <div class="col2">
...
{if $birth_date or $deceased_date}
   <div class="col2">
 {if $birth_date}
