...
class testEditActivityLogPhoneCall(PyHttpTestCase):
    def setUp(self):
        global db
    ...
    def runTest(self):
        ...
        if cid :
            ...
            subject  = 'Logging Test Phone Call'
            ...
            if qid :
                ...
                params = [
                    ('''_qf_default''', '''Phonecall:next'''),
                    ('''subject''', '''Logging Test Phone Call'''),
                    ('''phone_number''', '''759369572'''),
                    ('''scheduled_date_time[M]''', '''8'''),
                    ('''scheduled_date_time[d]''', '''25'''),
                    ('''scheduled_date_time[Y]''', '''2005'''),
                    ('''scheduled_date_time[h]''', '''11'''),
                    ('''scheduled_date_time[i]''', '''15'''),
                    ('''scheduled_date_time[A]''', '''AM'''),
                    ('''duration_hours''', '''4'''),
                    ('''duration_minutes''', '''0'''),
                    ('''status''', '''Left Message'''),
                    ('''details''', '''Logging Test Phone Call Edited Successfully'''),
                    ('''_qf_Phonecall_next''', '''Save'''),]
                url = "%s/civicrm/contact/view/activity" % drupal_path
                self.msg("Testing URL: %s" % url)
                Validator.validateRequest(self, self.getMethod(), "post", url, params)
                self.post(url, params)
                ...
                Validator.validateResponse(self, self.getMethod(), url, params)
                print "****************************************************************"
                print "Activity \'%s\' Edited Successfully." % subject
                print "****************************************************************"
            else :
                print "****************************************************************"
...
if __name__ == 'main':
    test = testEditActivityLogPhoneCall("testEditActivityLogPhoneCall")
    test.Run()
