...
class testActionAddToHouseHold(PyHttpTestCase):
    def setUp(self):
        global db
    ...
    def runTest(self):
        ...
        params = [
            ('''_qf_AddToHousehold_display''', '''true'''),]
        url = "%s/civicrm/contact/search/basic" % drupal_path
        self.msg("Testing URL: %s" % url)
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
        ...
        params = [
            ('''_qf_default''', '''AddToHousehold:next'''),
            ('''name''', ''''''),
            ('''_qf_AddToHousehold_refresh''', '''Search'''),]
        url = "%s/civicrm/contact/search/basic" % drupal_path
        self.msg("Testing URL: %s" % url)
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
        ...
        params = [
            ...
            ('''name''', ''''''),
            ('''contact_check''', '''15'''),
            ('''_qf_AddToHousehold_next''', '''Add To Household'''),]
        url = "%s/civicrm/contact/search/basic" % drupal_path
        self.msg("Testing URL: %s" % url)
        Validator.validateRequest(self, self.getMethod(), "post", url, params)
        self.post(url, params)
        ...
        url = "%s/civicrm/i/Inform.gif" % drupal_path
        self.msg("Testing URL: %s" % url)
        params = None
        Validator.validateRequest(self, self.getMethod(), "get", url, params)
        self.get(url, params)
...
if __name__ == 'main':
    test = testActionAddToHouseHold("testActionAddToHouseHold")
    test.Run()
