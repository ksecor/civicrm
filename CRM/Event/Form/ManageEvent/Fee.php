...
class CRM_Event_Form_ManageEvent_Fee extends CRM_Event_Form_ManageEvent
{
    ...
    public function buildQuickForm( ) 
    {
        ...
        for($i = 1; $i <= self::NUM_DISCOUNT; $i++) {
            ...
            if ( $i > 1 && 
                 ...
                 array_values( $this->_submitValues['discount_start_date'][$i] )
                 ) {
                $start_date = CRM_Utils_Date::format( $this->_submitValues['discount_start_date'][$i], '-' );
                if (! empty( $this->_submitValues['discount_start_date'][$i]['M'] ) 
                    && empty( $this->_submitValues['discount_end_date'][$i-1]['M'] ) )  {
                    $this->_submitValues['discount_end_date'][$i - 1] = 
                        CRM_Utils_Date::unformat( date('Y-m-d', strtotime ("-1 days $start_date") ));
                }
            }
    ...
    public function postProcess()
    {
        ...
        if ( $this->_id ) {
            ...
            if ( ! CRM_Core_BAO_PriceSet::removeFrom( 'civicrm_event', $eventId ) ) {
                require_once 'CRM/Core/OptionGroup.php';
                CRM_Core_OptionGroup::deleteAssoc( "civicrm_event.amount.{$eventId}" );
                }
        }
        ...
        if ( $params['is_monetary'] ) {
            ...
            if ( $params['price_set_id'] ) {
                CRM_Core_BAO_PriceSet::addTo( 'civicrm_event', $eventId, $params['price_set_id'] );
            } else {
                ...
                if ( ! CRM_Utils_System::isNull( $labels ) && ! CRM_Utils_System::isNull( $values )) {
                    ...
                    if ( ! empty( $options ) ) {
                        ...
                        CRM_Core_OptionGroup::createAssoc( "civicrm_event.amount.{$eventId}",
                                                           $options,
                ...
                if ( $this->_action & CRM_Core_Action::UPDATE ) {
                    //hack for CRM-3088
                    CRM_Core_OptionGroup::deleteAssoc ("civicrm_event.amount.{$eventId}.discount.%", "LIKE");
                }
                        ...
                        for ( $j = 1; $j <= self::NUM_DISCOUNT; $j++ ) {
                            ...
                            if ( ! empty( $discountOptions ) ) {
                                ...
                                $discountOptionsGroupId = 
                                    CRM_Core_OptionGroup::createAssoc( "civicrm_event.amount.{$eventId}.discount.{$params[discount_name][$j]}",
                                                                       $discountOptions,
