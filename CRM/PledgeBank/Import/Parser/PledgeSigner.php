 ...
 */
class CRM_PledgeBank_Import_Parser_PledgeSigner extends CRM_PledgeBank_Import_Parser
{
    ...
    function init( )
    {
        require_once 'CRM/PledgeBank/BAO/PledgeSigner.php';
        $fields =& CRM_PledgeBank_BAO_PledgeSigner::importableFields( $this->_contactType, false );
        $fields['event_id']['title'] = "Event ID";
        require_once 'CRM/Event/BAO/Event.php';
        $eventfields =& CRM_PledgeBank_BAO_Pledge::fields() ;
    ...
    function summary( &$values )
    {
        ...
        return CRM_PledgeBank_Import_Parser::VALID;
    }
    ...
    function import( $onDuplicate, &$values )
    {
        ...
        if ( $response != CRM_PledgeBank_Import_Parser::VALID ) {
            return $response;
        }
        $params =& $this->getActiveFieldParams( );
        $session =& CRM_Core_Session::singleton();
        ...
        if ( ! $params['participant_role_id'] ) {
            ...
            $params['participant_role_id'] = CRM_PledgeBank_PseudoConstant::participantRole( $roleId );
        } 
        //date-Format part ends
        static $indieFields = null;
        if ($indieFields == null) {
            require_once('CRM/PledgeBank/BAO/PledgeSigner.php');
            $indieFields =& CRM_PledgeBank_BAO_PledgeSigner::import();
        }
        ...
        if ( !CRM_Utils_Rule::integer($formatted['event_id']) ) {
            ...
            return CRM_PledgeBank_Import_Parser::ERROR;
        }
        if ( $formatError ) {
            array_unshift($values, $formatError['error_message']);
            return CRM_PledgeBank_Import_Parser::ERROR;
        }
        if ( $onDuplicate != CRM_PledgeBank_Import_Parser::DUPLICATE_UPDATE ) {
        ...
        if ( $onDuplicate == CRM_Event_Import_Parser::DUPLICATE_UPDATE ) {
            if ( $values['participant_id'] ) {
                require_once 'CRM/PledgeBank/BAO/Participant.php';
                $dao =  new CRM_PledgeBank_BAO_Participant();
                ...
                if ( $dao->find( true ) ) { 
                    ...
                    $newParticipant =& CRM_PledgeBank_BAO_PledgeSigner::create( $formatted , $ids );
        ...
        if ( is_array( $newParticipant ) && civicrm_error( $newParticipant ) ) {
            if ($onDuplicate == CRM_PledgeBank_Import_Parser::DUPLICATE_SKIP){
                array_unshift($values, $newParticipant['message']);
                if ( $newParticipant['error_message']['params'][0] ) {
                    return CRM_PledgeBank_Import_Parser::DUPLICATE;
                }
                return CRM_PledgeBank_Import_Parser::ERROR;
            } 
        }
