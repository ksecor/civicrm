...
/*
 +--------------------------------------------------------------------+
 | Elavon (Nova) Virtual Merchant Core Payment Module for CiviCRM version 2.x             |
 +--------------------------------------------------------------------+
	...
	* Send the data out over the wire
     ...
     *=============*/
      ...
      if  ($this->_mode == 'test')  {
            ...
            $params['trxn_id'] = $processorResponse['ssl_txn_id'];//'trxn_id' is varchar(255) field. returned value is length 37
        }
   ...
   function checkConfig( )                // CiviCRM V2.0 Declaration
   {
      ...
      function buildXML($requestFields){
   	...
   	//  $xml .= '<' . $key . '>' . rawurlencode($value) . '</' . $key . '>';
   	  ...
   	  $xml .= '<ssl_salestax>0.00</ssl_salestax></txn>';//sales tax appears to be required even though the dev guide says it isn't - not used by civi
 ...
 /************************************************************************
   ...
   ************************************************************************/
   function GetNodeValue($NodeName, &$strXML)
   {
     ...
     // echo " $NodeName &nbsp &nbsp $return <br>";// check out rtn values for debug
      return ($return);
   }
...
#####################################################################################
	...
	return  $processorResponse;
     ...
     }// end class CRM_Core_Payment_Elavon 
