...
//      }
    ...
    function &languages()
    {
        ...
        if (!$languages) {
            $languages = array('en_US' => 'English (USA)',
                               'af_ZA' => 'Afrikaans',
                               'bg_BG' => 'български',
                               'ca_ES' => 'Català',
                               'da_DK' => 'dansk',
            ...
            // check which ones are available; add them to $languages if not there already
            $config =& CRM_Core_Config::singleton();
            $available = array();
            if (is_dir($config->gettextResourceDir)) {
                ...
                while ($filename = readdir($dir)) {
                    if (preg_match('/^[a-z][a-z]_[A-Z][A-Z]$/', $filename)) {
                        $available[] = $filename;
                        if (!isset($languages[$filename])) $languages[$filename] = $filename;
                    }
                }
                closedir($dir);
            }
            ...
            foreach (array_keys($languages) as $code) {
                ...
                if (!in_array($code, $available)) unset($languages[$code]);
            }
        }
        return $languages;
    }
