...
class CRM_Core_BAO_UFGroup extends CRM_Core_DAO_UFGroup {
    ...
    static function createUFJoin( &$params, $ufGroupId ) 
    {
        ...
        $weight = CRM_Core_BAO_UFGroup::getWeight( );
        ...
        foreach ($allUFGroupType as $key => $value) {
            ...
            if (array_key_exists($key, $params) && !in_array($key, $ufGroupRecord )) {
                $joinParams['weight'] = $weight;
                // insert a new record
                CRM_Core_BAO_UFGroup::addUFJoin($joinParams);
            } else if (!array_key_exists($key, $params) && in_array($key, $ufGroupRecord) ) {
    ...
    static function getWeight ( ) 
    {
        ...
        $sqlString = "SELECT ( MAX(civicrm_uf_join.weight)+1) as new_weight
