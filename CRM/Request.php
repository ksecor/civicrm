...
/**
 * CRM_Request encapsulate an incoming request (GET/POST vars, session, cookies)
...
class CRM_Request {
  ...
  function initVars($module) {
    ...
    $pageID = CRM_Array::value( 'pageID', $_POST );
    if ($pageID)
    ...
    if (! empty($missingVars)) {
      CRM_Exception::throwCRMException("Missing required variables",
				       $missingVars);
    }
  ...
  function processVar( $name, $value ) {
    $var = CRM_Array::value( $name, self::$_vars );
    ...
    if (! CRM_Type::valid($type, $value, self::$_dateFormat)) {
      return;
    }
    ...
    if ($validVars[$name]) {
      $value = CRM_Type::filter($type, $value);
      $value = CRM_Type::format($type, $value);

      if ($name == 'action') {
	self::$_vars[$name]['value'] =
	  CRM_ObjectAction::getObjectAction();
      } else { 
  ...
  function callRule($name, $type, $min, $max, $value) {
    ...
    switch ($type) {
    case CRM_Type::TYPE_INT:
    case CRM_Type::TYPE_DOUBLE:
      return ($value >= $min && $value <= $max) ? true : false;

    case CRM_Type::TYPE_STRING:
      return CRM_Array::value( $value, $max ) ? true : false;
    }
  ...
  function restoreVars( $module ) {
    ...
    foreach ( self::$_vars as $name => $var ) {
      $storageName = CRM_Array::value('storageName', $var);
      $value = CRM_Array::value('value', $var);
  ...
  function get( $name ) {
    ...
    return CRM_Array::value('value', $var);
  }
