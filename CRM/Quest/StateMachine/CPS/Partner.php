 ...
 *
 */
class CRM_Quest_StateMachine_CPS_Partner extends CRM_Quest_StateMachine_CPS {
    ...
    static public function setPages( &$pages, &$stateMachine, &$controller ) {
        $pages['CRM_Quest_Form_CPS_Partner_PartnerIntro'] = null;
        $partners =& self::partners( );

        $dynamic = array( 'Princeton' => 'PrApplicant' );
        foreach ( $dynamic as $d => $v ) {
            require_once "CRM/Quest/Form/CPS/Partner/$d/$v.php";
            eval( '$newPages =& CRM_Quest_Form_CPS_Partner_' . $d . '_'  . $v . '::getPages( $controller );' );
        ...
        foreach ( $partners as $name => $values ) {
            if ( $validPartners[$values['title']] ) {
                foreach ( $values['steps'] as $key => $title ) {
                    $pages["{$name}-{$key}"] = array( 'className' => "CRM_Quest_Form_CPS_Partner_{$name}_{$key}",
                                                      'title'     => $title,
                                                      'options'   => array( ) );
                }
            }
        }
        $pages['CRM_Quest_Form_CPS_Partner_PartnerSubmit'] = null;
    }
