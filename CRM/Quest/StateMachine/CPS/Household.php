 ...
 *
 */
class CRM_Quest_StateMachine_CPS_Household extends CRM_Quest_StateMachine_CPS {
    ...
    static public function setPages( &$pages, &$stateMachine, &$controller ) {
        $pages['CRM_Quest_Form_CPS_Household'] = null;

        $dynamic = array( 'Household', 'Sibling', 'Income' );
        foreach ( $dynamic as $d ) {
            require_once "CRM/Quest/Form/CPS/$d.php";
            eval( '$newPages =& CRM_Quest_Form_CPS_' . $d . '::getPages( $controller );' );
            $pages = array_merge( $pages, $newPages );
        }

        if ( self::includeNonCustodial( $stateMachine, $controller ) ) {
            $pages['CRM_Quest_Form_CPS_Noncustodial'] = null;
        }
    }
