 ...
 *
 */
abstract class CRM_Quest_StateMachine_CPS extends CRM_Core_StateMachine {
                ...
                foreach ( $this->_pageNames as $pageName ) {
                    if ( substr( $pageName, 0, strlen( $name ) ) == $name ) {
                        if ( ! $data['valid'][$pageName] ) {
                            ...
                            CRM_Utils_System::redirect( CRM_Utils_System::url( 'civicrm/quest/cps/' . strtolower( $controller->_subType ),
    ...
    public function isApplicationComplete( &$controller ) {
        ...
        foreach ( $this->_pageNames as $name ) {
            $valid = CRM_Quest_Controller_CPS::isPageAlwaysValid( $name ) ? 1 : $data['valid'][$name];
            if ( ! $valid ) {
                return false;
            }
        }
