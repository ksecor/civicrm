...
/**
 * This class is used for building the Household form. This class also has the actions that should be done when form is processed.
 * 
...
class CRM_Contact_Form_Household extends CRM_Form 
{
    ...
    function setDefaultValues( ) 
    {
        ...
        if ( $this->_mode & ( self::MODE_VIEW | self::MODE_UPDATE ) ) {
            ...
            $contact_household->get("contact_id",$lng_contact_id);

            $defaults['household_name'] = $contact_household->household_name;
            $defaults['nick_name'] = $contact_household->nick_name;
            $defaults['primary_contact_id'] =  $contact_household->primary_contact_id; 
    ...
    /**
     * This function provides the HTML form elements for the add/view/update operation of Household.
     * 
    ...
    /**
     * This function does all the processing of the form for New Household
     * Depending upon the mode this function is used to insert or update the Household
     * @access private
     */
    ...
    { 
        ...
        if (!strlen($str_error)) { //proceed if there are no errors
            ...
            $contact_household->household_name = $a_Values['household_name'];
            $contact_household->nick_name = $a_Values['nick_name'];
            $contact_household->primary_contact_id = $a_Values['primary_contact_id'];
 
            if ($lng_contact_id) {
                // update the contact_household for $lng_contact_id
                $contact_household->whereAdd('contact_id = '.$lng_contact_id);
                if(!$contact_household->update(DB_DATAOBJECT_WHEREADD_ONLY)) $str_error = mysql_error();
            } else {
                // insert in contact_household
                if (!$contact_household->insert()) $str_error = mysql_error();                
            }
        }
        ...
        if (strlen($str_error)) { //commit if there are no errors else rollback
            ...
            form_set_error('household_name', t('Contact Household has been saved successfully.'));
        }
