...
* @package JPSpan
* @subpackage Include
* @version $Id: Include.php,v 1.5 2004/11/23 13:23:10 harryf Exp $
*/
//-----------------------------------------------------------------------------
...
function JPSpan_Include_ErrorReader($lang='en',$app=array(),$ser=array(),$cli=array()) {
    ...
    if ( !file_exists('packages/JPSpan/errors/'.$errorfile) ) {
        $errorfile = 'errors.en.ini';
    }
    
    //$errors = parse_ini_file(JPSPAN . 'errors/'.$errorfile,TRUE);
    $errors = parse_ini_file('packages/JPSpan/errors/'.$errorfile,TRUE);
    ...
    */
    function getCode() {
        if ( JPSPAN_INCLUDE_COMPRESS ) {
            ...
            require_once 'packages/JPSpan/Script.php';
    ...
    */
    function inc($file, $state) {
        if ( $state == JPSPAN_LEXER_UNMATCHED ) {
            ...
            $this->includes[] = 'packages/JPSpan/js/' . trim($file);
...
class JPSpan_Include_Parser {
    ...
    */
    function & getLexer() {
        ...
        require_once 'packages/JPSpan/Lexer.php';
