	...
	},
	{
		...
		_performSearch: function(/*String*/ searchStr, /*Function*/ callback){
			...
			for(var x=0; x<this._data.length; x++){
				...
				var dataLabel = new String((!this.caseSensitive && typeof(this._data[x][0])=='string') ? this._data[x][0].toLowerCase() : this._data[x][0].toString());
	...
	dojo.widget.HtmlWidget,
	{
		...
		_openResultList: function(/*Array*/ results){
			...
			if(	(this.autoComplete)&&
				...
				if((cpos+1) > this.textInputNode.value.length){
					...
					this.textInputNode.value += results[0][0].toString().substr(cpos);
		...
		_handleBlurTimer: function(/*Boolean*/clear, /*Number*/ millisec){
			...
			if(millisec){ // we ignore that zero is false and never sets as that never happens in this widget
				//console.log('should be setting timeout to checkblurred');
		...
		_isInputEqualToResult: function(/*String*/ result){
			...
			if(!this.dataProvider.caseSensitive && typeof(this.textInputNode.value)=='string' && typeof(result)=='string'){
		...
		_checkBlurred: function(){
			//console.log('_checkBlurred is firing');
			//console.log('foucs: '+this._hasFocus);
			//console.log('MOUSEOVER list: '+this._mouseover_list);
			if(!this._hasFocus && (typeof(this._mouseover_list)=='undefined' || !this._mouseover_list)){
				//console.log('inside first check');
				this._hideResultList();
				//console.log('after hiding');
				// clear the list if the user empties field and moves away.
				if(!this.textInputNode.value.length){
					//console.log('inside !this.textInputNode.value.length');
					this.setAllValues("", "");
					return;
				}
				...
				if(this.forceValidOption && !isValidOption){
					//console.log('inside this.forceValidOption && !isValidOption');
					this.setAllValues("", "");
					return;
				}
				if(!isValidOption){// clear
					//console.log('check for valid option?:' +isValidOption);
					this.setSelectedValue("");
				}else{
					var tgt = dojo.html.firstElement(this.optionsListNode);
					while(tgt){
						if(this._isInputEqualToResult(tgt.getAttribute("resultName"))){
							if(this.comboBoxSelectionValue.value!=tgt.getAttribute("resultName") && this.comboBoxSelectionValue.value!=tgt.getAttribute("resultValue")){
								this.textInputNode.value = "";
								//console.log('inside this this.comboBoxSelectionValue.value!=tgt.getAttribute(resultName)');
